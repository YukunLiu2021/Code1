# This is the main script for the DC test
...
dftestRaw.to_csv('PreAdded.csv')#Save all as a whole
dftestRaw['FLAG_PRE'].to_csv('FlagPrediction.csv')#Save the predictions as a separate csv

# (6) Give a brief summary of the output
PreCount=Counter(FlagPre)
print('Summary of the predicted TARGET_FLAG:',PreCount[1],'clients would claim,',PreCount[0],'clients would not claim')
print("Predictions are appended as a column 'FLAG_PRE' with other variables, saved as 'PreAdded.csv', and separatly as 'FlagPrediction.csv'")
